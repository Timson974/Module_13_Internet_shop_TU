...
export class OrderComponent implements OnInit {
  ...
  dialogRref: MatDialogRef<any> | null = null;

  constructor(private cartService: CartService, private router: Router, private orderService: OrderService,
              private _snackBar: MatSnackBar, private fb: FormBuilder, private userService: UserService,
  ...
  createOrder() {
    ...
    && this.orderForm.value.phone && this.orderForm.value.phone && this.orderForm.value.paymentType
      ...
      this.orderService.createOrder(paramsObject)
        .subscribe({
          next: (data: OrderType | DefaultResponseType) => {
            ...
            this.dialogRref = this.dialog.open(this.popup);
            this.dialogRref.backdropClick()
              .subscribe(() => {
                this.router.navigate(['/']);
              });
          ...
          error: (errorResponse: HttpErrorResponse) => {
            if (errorResponse.error && errorResponse.error.message) {
              ...
              this._snackBar.open('Ошибка при заказе');
            }
          }
        });
    } else {
